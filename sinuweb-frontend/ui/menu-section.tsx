...
function ListMenuSection({menuTitle, links}: {menuTitle: any, links: any}) {
    return ( 
        <div className="flex-1 text-left">
            <h3 className="border-b border-slate-500 py-5">{menuTitle}</h3>
            ...
            <ul className="flex flex-col gap-3 text-[15px] font-light py-5">
                {links.map((link: any) => 
                    <li 
                        key={link.title}
                        >
                        <Link className="hover:text-slate-200 hover:underline" href={link.url}>
                            {link.title}
                        </Link>
                    </li>
                )}
